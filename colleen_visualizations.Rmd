...
```{r}
#making new dataset with percentages (b/c that would make more sense to visualize so that they are scaled to the population)
newdata <- data %>%
...
ggplot(newdata, aes(fill = percent_white)) + 
  geom_sf(color = NA) + 
...
ggplot(newdata, aes(fill = perc_internet_sub)) + 
  geom_sf(color = NA) + 
...
ggplot(newdata, aes(fill = perc_pub_assist)) + 
  geom_sf(color = NA) + 
  scale_fill_viridis_c(option = "magma", labels = scales::percent) +
...
ggplot(newdata, aes(fill = TOTALRE)) + 
  geom_sf(color = NA) + 
  scale_fill_viridis_c(option = "magma", labels = scales::dollar) +
